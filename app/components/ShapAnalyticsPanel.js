...
RETENTION COMPLIANCE SECTOR // CHURN.AI PLATFORM`;
    ...
    return (
        <div className="border border-zinc-900 rounded-xl p-5 bg-zinc-900/10 font-mono text-[11px] space-y-5 animate-in fade-in duration-200">

            {/* 🏢 FEATURE 1: CUSTOMER 360° PROFILE SCHEMATICS CONTAINER HEADER */}
            <div className="border-b border-zinc-900 pb-3 space-y-2">
                <div className="flex justify-between items-start">
                    <div>
                        <span className="text-zinc-500 font-bold uppercase text-[9px]">{"// CUSTOMER_360_VIEW"}</span>
                        <h4 className="text-zinc-100 text-xs font-black tracking-tight uppercase mt-0.5">{activeNode.accountNode}</h4>
                    </div>
                ...
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 pt-1.5 text-[10px] text-zinc-400 border-t border-zinc-900/40">
                    <div>INDUSTRY SINK: <span className="text-zinc-200">{activeNode.industry}</span></div>
                    <div>CONTRACT ALLOC: <span className="text-zinc-200">${Number(activeNode.monthlySpending).toFixed(0)}/mo</span></div>
                    <div>COMPLAINTS FILE: <span className="text-zinc-200">{activeNode.complaintCount} TICKET LOGS</span></div>
                    <div>INACTIVITY GAP: <span className="text-zinc-200">{activeNode.inactivityDays} DAYS IDLE</span></div>
                </div>
            </div>

            {/* 📈 FEATURE 2: HISTORICAL CHURN TIMELINE TRACKER LINE */}
            <div className="space-y-2 bg-zinc-950/30 p-3 rounded-lg border border-zinc-900/50">
                <span className="text-[9px] text-zinc-400 font-bold tracking-tight block">{"// HISTORICAL_CHURN_TIMELINE_TRENDS"}</span>
                <div className="flex items-center justify-between text-[10px] text-zinc-500 pt-1">
                    {predictionLogsList.length > 0 ? (
                        predictionLogsList.slice(0, 4).reverse().map((log, index) => (
                            <div key={index} className="flex items-center gap-2">
                                <div className="text-center">
                                    <div className="text-[8px] text-zinc-600">RUN {predictionLogsList.length - (predictionLogsList.length - 1 - index)}</div>
                                    <div className={`font-bold mt-0.5 ${log.churnProbability >= 0.65 ? "text-rose-400" : log.churnProbability >= 0.35 ? "text-amber-400" : "text-emerald-400"}`}>
                                        {(log.churnProbability * 100).toFixed(0)}%
                                    </div>
                                </div>
                                {index < Math.min(predictionLogsList.length, 4) - 1 && <span className="text-zinc-800">→</span>}
                            </div>
                        ))
                    ) : (
                        <span className="text-zinc-600 italic text-[10px] pl-0.5">No evaluation matrices calculated for selected target.</span>
                    )}
                </div>
            </div>

            {/* Aggregate Score Matrix Meter */}
            <div className="space-y-1.5 bg-zinc-950/40 p-3 rounded-lg border border-zinc-900/60">
                <div className="flex justify-between text-[10px] text-zinc-400">
                    <span>CHURN PROBABILITY MODEL EXPOSURE</span>
                    <span className="font-bold text-zinc-200">{probabilityPercent}%</span>
                </div>
                <div className="w-full bg-zinc-900 h-1.5 rounded-full overflow-hidden">
                    <div
                        className={`h-full transition-all duration-500 ease-out ${baseRiskProbabilityScore >= 0.65 ? "bg-rose-500" : baseRiskProbabilityScore >= 0.35 ? "bg-amber-500" : "bg-emerald-500"
                            }`}
            ...
            <div className="space-y-3.5">
                <span className="text-[10px] text-zinc-500 uppercase tracking-wider block">Local Feature Weight Vectors:</span>
                    ...
                    {weights.map((item, idx) => {
                        ...
                        return (
                            <div key={idx} className="space-y-1">
                                ...
                                <div className="w-full bg-zinc-950/50 h-3 border border-zinc-900/40 rounded relative overflow-hidden flex">
                                    <div className="w-1/2 border-r border-zinc-900 h-full flex justify-end">
                                        {!isPositive && (
                                            <div
                                                className="bg-emerald-500/20 border-y border-l border-emerald-500/50 h-full transition-all duration-300"
                                    ...
                                    <div className="w-1/2 h-full">
                                        {isPositive && (
                                            <div
                                                className="bg-rose-500/20 border-y border-r border-rose-500/50 h-full transition-all duration-300"
                                                style={{ width: `${absoluteWidth}%` }}
                                            />
                                        )}
                                    </div>
            ...
            <div className="space-y-3 bg-zinc-950/50 p-3 rounded-lg border border-zinc-900/60">
                ...
                <div className="space-y-2">
                    <div className="space-y-1">
                        ...
                        <input type="range" min="0" max="10" value={complaints} onChange={(e) => setComplaints(parseInt(e.target.value, 10))} className="w-full accent-zinc-500 h-1 bg-zinc-900 rounded cursor-pointer" />
                    </div>
                    ...
                    <div className="space-y-1">
                        ...
                        <input type="range" min="0" max="30" value={inactivity} onChange={(e) => setInactivity(parseInt(e.target.value, 10))} className="w-full accent-zinc-500 h-1 bg-zinc-900 rounded cursor-pointer" />
                    </div>
                    ...
                    <div className="space-y-1">
                        ...
                        <input type="range" min="0" max="50" value={frequency} onChange={(e) => setFrequency(parseInt(e.target.value, 10))} className="w-full accent-zinc-500 h-1 bg-zinc-900 rounded cursor-pointer" />
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-2">
                ...
                <div className="relative">
                    <textarea
                        ...
                        className="w-full h-24 bg-zinc-950/80 border border-zinc-900 text-zinc-500 text-[9px] p-2.5 rounded font-mono leading-normal resize-none focus:outline-none select-all"
                    />
                ...
                <div className="pt-2 space-y-2">
                    ...
                    <div className="space-y-2">
                        {playbookRecommendations.map((rec, i) => (
                            <div key={i} className="border border-zinc-900 bg-zinc-900/20 rounded p-2.5 space-y-0.5">
            ...
            <div className="pt-2 border-t border-zinc-900/60 flex justify-between text-[9px] text-zinc-500 select-none">
